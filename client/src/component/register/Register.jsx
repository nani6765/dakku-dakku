...
function Register() {
  ...
  const registerFunc = async (e) => {
    ...
    if(email && password && passwordChk && displayName && (password == passwordChk)) {
      ...
      try {
        ...
        .catch((err)=>{console.log("에러입니다", err.code.split("/")[1])});
        ...
        await axios.post("http://localhost:5000/api/user/register", user).then((res) => {
          if (res.data.success) {
            console.log("success");
            alert("회원가입 완료");
  ...
  return (
    <RegisterBox>
      <form action='submit'>
        ...
        <br/>
        <Input type="text" placeholder='비밀번호 확인' value={passwordChk} onChange={(e) => {setPasswordChk(e.target.value)}} />
        <br/>
        ...
        <GotoLogin onClick={() => {navigate('/login')}}>로그인 이동</GotoLogin>
      </form>
    </RegisterBox>
  )
}
